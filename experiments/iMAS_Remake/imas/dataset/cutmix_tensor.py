...
def mixup_detection(images, targets, lam=None):
    if lam is None:
        lam = np.random.beta(4, 4)
    permutation = torch.randperm(images.size(0))
    mixed_images = images * lam + images[permutation] * (1.0 - lam)
    mixed_targets = []
    target_list = _as_box_list(targets)
    for index, target in enumerate(target_list):
        other = target_list[permutation[index].item()] if index < len(target_list) else target
        if isinstance(target, torch.Tensor) and isinstance(other, torch.Tensor):
            mixed_targets.append(torch.cat([target, other], dim=0))
        else:
            mixed_targets.append(target)
    return mixed_images, mixed_targets


def cutmix_detection(images, targets, lam=None):
    if lam is None:
        lam = np.random.beta(4, 4)
    permutation = torch.randperm(images.size(0))
    mixed_images = images.clone()
    target_list = _as_box_list(targets)
    bbx1, bby1, bbx2, bby2 = rand_bbox(images.size(), lam=lam)
    for index in range(images.size(0)):
        mixed_images[index, :, bbx1[index]:bbx2[index], bby1[index]:bby2[index]] = images[
            permutation[index], :, bbx1[index]:bbx2[index], bby1[index]:bby2[index]
        ]
    mixed_targets = []
    for index, target in enumerate(target_list):
        other = target_list[permutation[index].item()] if index < len(target_list) else target
