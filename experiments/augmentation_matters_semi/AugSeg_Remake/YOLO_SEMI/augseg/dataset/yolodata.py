...
def build_semi_yololoader(split, all_cfg, seed=0):
    ...
    # AugSeg original logic: oversample labeled to match unlabeled count.
    # Count actual unlabeled samples from list file.
    data_list_unsup = cfg["data_list"].replace("/labeled/", "/unlabeled/")
    with open(data_list_unsup, "r") as f:
        n_unsup = sum(1 for line in f if line.strip())
    n_sup = n_unsup  # oversample labeled data to the amount of unlabeled data

    # build transform
    mean, std = cfg["mean"], cfg["std"]
    trs_form_weak = build_basic_transfrom(cfg, split=split, mean=mean)
    # AugSeg Origin: TIBA applied in dataset on unlabeled strong view,
    # then ALIA applied in training loop on the already-TIBA'd tensor.
